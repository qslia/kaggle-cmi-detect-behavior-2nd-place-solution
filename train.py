...
def process_sequence(sid, df, label2idx):
    ...
    feat = grp.select(["acc_x", "acc_y", "acc_z", "rot_x",
                      "rot_y", "rot_z", "rot_w", "handedness"]).to_numpy()
...
class GestureDataset(Dataset):
    ...
    def __init__(self, df: pl.DataFrame, seq_ids: Sequence[Any], label2idx: Dict[str, int],
        ...
        self.samples: list[tuple[np.ndarray,
                                 np.ndarray, int, int, np.ndarray, str]] = []
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=36) as executor:
            ...
            futures = [executor.submit(
                process_sequence, sid, df, self.label2idx) for sid in self.seq_ids]
    ...
    def _mixup_by_phase(self, x1, phases1, x2, phases2, lam):
        ...
        for phase in [0, 1, 2]:
            ...
            if len(idx1) == 0 and len(idx2) == 0:
                ...
                mixed_phase_segments.append(torch.full(
                    (len(idx2),), phase, dtype=torch.long))
            elif len(idx2) == 0:
                # If x2 has no such phase, use x1 phase as is
                seg1 = x1[idx1]
                mixed_segments.append(seg1)
                mixed_phase_segments.append(torch.full(
                    (len(idx1),), phase, dtype=torch.long))
            else:
...
def collate_fn(batch):
    ...
    padded_phases = torch.full(
        (len(batch), max_len), -1, dtype=torch.long)  # -1 for padding
...
class GestureLitModel(L.LightningModule):
    def __init__(
        ...
        hidden_size: int = 128,
        num_layers: int = 4,
        # number of classes for (orientation × gesture × phase1_behavior)
        num_classes: int = 144,
        lr: float = 1e-3,
        weight_decay: float = 1e-4,
        model_type: str = "squeezeformer",  # "squeezeformer" or "gru"
        # mapping used for evaluation
        eval_mapping: Dict[str, Any] | None = None,
    ) -> None:
        super().__init__()
        # exclude eval_mapping from saved hparams
        self.save_hyperparameters(ignore=['eval_mapping'])
    ...
    def _shared_step(self, batch, stage: str):
        ...
        if isinstance(outputs, dict) and 'phase_logits' in outputs:
            phase_criterion = nn.CrossEntropyLoss(
                ignore_index=-1)  # -1 is padding
            phase_loss = phase_criterion(
                outputs['phase_logits'].view(-1, 3), phases.view(-1))
            loss = loss + phase_loss
            self.log(f"{stage}/phase_loss", phase_loss,
    ...
    def on_validation_epoch_end(self):
        ...
        if self.eval_mapping is not None:
            ...
            bin_targets = (targets_72 < 36).astype(
                int)  # provisional threshold
...
class CFG:
    ...
    # "imu" or "all" or "imu_simple" or "all_simple" or "imu_deep" or "all_25d"
    model_type: str = "imu"
    ...
    # probability to apply TOF mask augmentation
    use_tof_mask_augmentation_prob: float = 0.1
...
def prepare_dataframe(csv_path: str) -> pl.DataFrame:
    ...
    df_demo = pl.read_csv(
        "cmi-detect-behavior-with-sensor-data/train_demographics.csv")
...
def get_phase1_behavior(df: pl.DataFrame, sequence_id: str) -> str:
    ...
    phase1_behaviors = ['Moves hand to target location',
...
def make_label_mapping(df: pl.DataFrame) -> Dict[Tuple[str, str, str], int]:
    ...
    print(
        f"Created {len(label2idx)} orientation-gesture-phase1behavior combinations")

    orientations = df0.select("orientation").unique().sort(
        "orientation").to_series().to_list()
...
def run_fold(cfg: CFG, fold_idx: int, splits: List[List[int]], label2idx: Dict[Tuple[str, str, str], int], seq_ids: List[str], df: pl.DataFrame) -> Dict[str, float]:
    ...
    use_tof = (cfg.model_type in [
               "all", "all_simple", "all_deep", "all_rot", "all_25d"])
    ...
    if cfg.use_mixup:
        ...
        train_ds = MixupDataset(
            train_ds, alpha=cfg.mixup_alpha, num_classes=len(label2idx))
    ...
    print(
        f"Eval mapping created: {len(eval_mapping['non_target_gestures'])} Non-Target gestures, {len(eval_mapping['target_gestures'])} Target gestures")
    ...
    model._logits_save_path = str(
        logits_save_path) if cfg.save_logits else None
